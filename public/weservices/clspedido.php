...
public static function llego_la_moto($id_pedido)
 ...
 public static function lista_pedido_por_id_moto_por_fecha($id_moto,$dia,$mes,$anio)
  ...
  {$resultado=-1;
    $consulta ="select c.id,c.direccion_inicio,d.latitud as 'latitud_inicio',d.longitud as 'longitud_inicio',c.direccion_fin,c.distancia,c.opciones,c.fecha_inicio as 'fecha1',c.fecha_fin as 'fecha2',c.id_pedido,c.id_usuario,c.id_moto,c.monto from carrera c, direccion d where d.id=c.direccion_inicio and  c.id_pedido= ? order by c.id desc limit 1";
    try{
      $comando=parent::getInstance()->getDb()->prepare($consulta);
      $comando->execute(array($id_pedido));
      $row=$comando->fetch(PDO::FETCH_ASSOC);
	  ...
	  {try{
	     ...
	     // obteniendo el empuje del objeto push
		   $push->set_empresa(self::get_empresa_por_id_usuario($id_usuario));
			 ...
			 $devicetoken = self::get_token_moto($latitud,$longitud,1000);		 
    ...
    public static function enviar_otras_notificacion_con_radio_ascendente($id_usuario,$id_pedido,$nombre)
    {
    	
    	$distancia=2000;
    	while ($distancia<=3000) {
    		sleep(20);
    		$consulta= parent::getInstance()->getDb()->prepare("SELECT * from pedido where id=? and id_moto is null limit 1");
	        $consulta->execute(array($id_pedido)); 
	 		$fila=$consulta->fetch(PDO::FETCH_ASSOC);
			if($fila)
			{
			$sw=self::enviar_otra_notificacion_pedido_moto($id_usuario,$fila['latitud'],$fila['longitud'],$id_pedido,$nombre,$distancia);
    ...
    public static function enviar_otra_notificacion_pedido_moto($id_usuario,$latitud,$longitud,$id_pedido,$nombre,$distancia)
	  {try{
		   $push = new Push('Pedido',$nombre.' a pedido una moto.',null,"moto",$id_pedido,$nombre,$latitud,$longitud,"2");
	     // obteniendo el empuje del objeto push
		   $push->set_empresa(self::get_empresa_por_id_usuario($id_usuario));
			 ...
			 $devicetoken = self::get_token_moto_otras_notificacion($latitud,$longitud,$distancia,$id_pedido);		 
	...
	public static function get_token_moto($latitud,$longitud,$distancia)
   { //distancia por default=8000  
   	$query = parent::getInstance()->getDb()->prepare("SELECT token,distancia_entre_dos_puntos(?,?,latitud,longitud) as distancia FROM moto where  estado=1 and login=1 and distancia_entre_dos_puntos(?,?,latitud,longitud)<= ? order by distancia asc");
         $query->execute(array($latitud, $longitud,$latitud, $longitud,$distancia)); 
         $tokens = array(); 
   ...
   public static function get_token_moto_otras_notificacion($latitud,$longitud,$distancia,$id_pedido)
   { //distancia por default=8000  
		$consulta= parent::getInstance()->getDb()->prepare("SELECT * from pedido where id=? and id_moto is null limit 1");
        $consulta->execute(array($id_pedido)); 
 		$fila=$consulta->fetch(PDO::FETCH_ASSOC);
