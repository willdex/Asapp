...
switch ($_GET['opcion']) {
	...
	case 'set_puntuacion':
		set_puntuacion();
	break;	
	case 'enviar_notificacion_de_pedido':
	     enviar_notificacion_de_pedido();
	break;
	default:
...
function get_pedido_por_id_moto()
{
  ...
  $credito=Pedido::get_credito_por_id_moto($dato['id_moto']);
  if($row!="-1")
  {
  	$resultado['suceso']="1";
    $resultado['mensaje']="Correcto.";
  ...
  {
   print json_encode(array("suceso"=>"2","mensaje"=>"No tiene pedidos habilitados.","credito"=>$credito  ));
  }
}
...
function enviar_notificacion_de_pedido()
{
	$dato=json_decode(file_get_contents("php://input"),true);
	$row=Pedido::enviar_otras_notificacion_con_radio_ascendente($dato['id_usuario'],$dato['id_pedido'],$dato['nombre']);

	if ($row===true) {
		$dato["suceso"]="1";
		$dato["mensaje"]="Notificacion enviada.";
		print json_encode($dato);
	}
	else
	{
		print json_encode(array('suceso' =>'2' ,'mensaje'=>'No se pudo realizar el envio de notificacion a los Motistas.' ));
	}	
}
function pedir_moto()
{$sw=false;
			...
			if($estado==1)
			{
				if(Pedido::get_estado_empresa_por_id_usuario($id_usuario)=="1")
				{
				  $p=Pedido::pedir_moto($id_usuario,$latitud,$longitud,"Por favor una moto.",$nombre,$id_empresa);
					if($p!="-1")
					{
					    	print json_encode(array('suceso' => '1','mensaje' => 'Su pedido se ha realizado correctamente.','id_pedido'=>$p));
