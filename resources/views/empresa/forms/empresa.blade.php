<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
<H2>REGISTRO DE EMPRESA</H2>    

    <div class="form-group">
        {!!Form::label('nombre_administrador','Nombre Administrador:')!!}
        {!!Form::text('nombre_administrador',null,['class'=>'form-control ','placeholder'=>'Ingrese el nombre'])!!}
    </div>
    <div class="form-group">
        {!!Form::label('apellido_administrador','Apellidos:')!!}
        {!!Form::text('apellido_administrador',null,['class'=>'form-control','placeholder'=>'Ingrese los apellidos'])!!}
    </div>

    <div class="form-group">
        {!!Form::label('telefono_administrador','Teléfono Del Adiministrador:')!!}
        {!!Form::text('telefono_administrador',null,['class'=>'form-control','placeholder'=>'Ingrese el telefono del administrador','onkeypress'=>'return bloqueo_de_punto(event)','maxlength'=>'8'])!!}
    </div>

    <div class="form-group">
        {!!Form::label('email','Email:')!!}
        {!!Form::text('email',null,['class'=>'form-control','placeholder'=>'Ingrese el email'])!!}
    </div>

		<input type="hidden" name="id_empresa" id="id_empresa" value="0">
       <?php /* <input type="text" name="latitud" id="latitud" value="0">
        <input type="text" name="longitud" id="longitud" value="0">*/ ?>
        <input type="hidden" name="id_casa" id="id_casa" value="0">
        <input type="hidden" name="id_trabajo" id="id_trabajo" value="0">
        <input type="hidden" name="token" id="token" value="0">


		<div class="form-group">
			{!!Form::label('nombre_empres','Nombre Empresa:')!!}
			{!!Form::text('nombre_empres',null,['class'=>'form-control','placeholder'=>'Ingrese el nombre de la empresa'])!!}
		</div>

		<div class="form-group" hidden="">
			{!!Form::label('direccion','Direccion:')!!}
			{!!Form::text('direccion','Ninguna',['class'=>'form-control','placeholder'=>'Ingrese la direccion'])!!}
		</div>

		<div class="form-group">
			{!!Form::label('telefono_empres','Telefono De La Empresa:')!!}
			{!!Form::text('telefono_empres',null,['class'=>'form-control','placeholder'=>'Ingrese el numero de la empresa','onkeypress'=>'return bloqueo_de_punto(event)','maxlength'=>'15'])!!}
		</div>

		<div class="form-group">
			{!!Form::label('razon_social','Razon Social:')!!}
			{!!Form::text('razon_social',null,['class'=>'form-control','placeholder'=>'Ingrese razon_social'])!!}
		</div>

		<div class="form-group">
			{!!Form::label('nit','NIT:')!!}
			{!!Form::text('nit',null,['class'=>'form-control','placeholder'=>'Ingrese el nit','onkeypress'=>'return bloqueo_de_punto(event)'])!!}
		</div>

		<div class="form-group" hidden="">
			{!!Form::label('latitud','Latitud:')!!}
			{!!Form::text('latitud',0,['class'=>'form-control','placeholder'=>'Ingrese la latitud'])!!}
		</div>

		<div class="form-group" hidden="">
			{!!Form::label('longitud','Longitud:')!!}
			{!!Form::text('longitud',0,['class'=>'form-control','placeholder'=>'Ingrese la longitud'])!!}
		</div>	

		<div class="form-group" hidden="">
			{!!Form::label('credito','Credito:')!!}
			{!!Form::text('credito',null,['class'=>'form-control','placeholder'=>'Ingrese el credito','onkeypress'=>'return bloqueo_de_punto(event)','maxlength'=>'3'])!!}
		</div>
		<input type="hidden" name="id_administrador" id="id_administrador" value="0">

</div>

<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12" align="center">
<br><br><br>
{!!Form::label('Foto Del Administrador:')!!} <br>
    <img id="logo" src="{{asset('images/usuario.jpg')}}" title="Cargar Imagen" style="cursor: pointer" onclick="cargarImagen(this, 1)" class="img-circle"/><br>
	{!!Form::hidden('imagen','data:image/jpeg;base64,',['id'=>'imagen','class'=>'form-control'])!!}    
</div>

<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
	<div id="map" style="width: 600px; height: 550px;"></div>
	<div class="pull-right">
	   <button id="btn_ver" type="button" class="btn-sm btn-info" hidden="" onclick="lista_direccion()">VER TODOS</button>	
	   <button id="bt_add" type="button" class="btn-sm btn-success" hidden="" onclick="nuevo()"><i class="fa fa-plus-square" aria-hidden="true"></i></button>
	</div>
<!--input type="text" name="coords" id="coords"-->


<?php /* <input type="text" name="latitud_aux" id="latitud_aux">
<input type="text" name="longitud_aux" id="longitud_aux">
*/?>

        <table class="table table-striped table-bordered table-condensed table-hover">
          <thead>
            <th><center>NOMBRE</center></th>
            <th><center>DIRECCION</center></th>
            <th><center>OPCION</th>           
          </thead>

          <tbody align="center" id="lista">
          </tbody>

        </table>

</div>


